...
export function WordLookup({ word, contextText, targetLang, children, className }: WordLookupProps) {
  ...
  const { speak } = useTTS();
  const { phonetic, meanings, translation, isLoading } = useWordDictionary(word, targetLang, open, contextText);
    ...
    <HoverCard open={open} onOpenChange={setOpen} openDelay={200} closeDelay={100}>
      ...
      <HoverCardContent className="p-4">
        {isLoading ? (
          ...
          <p className="text-xs text-slate-400">Không tìm thấy nghĩa cho “{word}”.</p>
        ) : (
